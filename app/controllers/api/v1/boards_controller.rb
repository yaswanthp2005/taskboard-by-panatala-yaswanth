...
class Api::V1::BoardsController < ApplicationController
  ...
  def create
    ...
    authorize board
    board.save!
    render_notice(t("successfully_created", entity: t("entities.board")), :ok)
  end
  ...
  private
    ...
    def load_board!
      @board = policy_scope(Board).includes(lists: { cards: [:assignees, :labels] }).find_by!(slug: params[:slug])
    end
